...
def process_station(base_dir, config):
    ...
    if len(peak_mus) == 1:
        ...
        print(f'\nSingle peak detected for {station_name}:')
        ...
        with open(calib_path, 'w') as f:
            f.write(f'# Single peak calibration point for {station_name.upper()}\n')
    ...
    if E0_VALUE is None:
        print(f'E0_VALUE is not set in the script for {station_name}. Please edit the file and set E0_VALUE = <numeric MeV>.')
        return
    E0 = float(E0_VALUE)
    ...
    for fn in valid_energies_labels:
        ...
        if scale is None:
            # attempt to parse a number like '1x_ar3' from filename
            if f'1x{station_name}' in fn:
                scale = 1.0
            elif f'0.5x{station_name}' in fn:
                scale = 0.5
    ...
    calib_path = os.path.join(base_dir, f'calibration_params_{station_name}.txt')
    with open(calib_path, 'w') as f:
        f.write(f'# Calibration parameters for {station_name.upper()}: channel = a * E + c, or E = (channel - c) / a\n')
        f.write(f'a = {a}\n')
