...
# Set font for plots (consistent with E_spec_nit.py)
...
# ============================================================================
# NITROGEN GAS PROPERTIES (for energy loss calculation)
# ============================================================================
Z_N = 7  # atomic number
A_N = 14.007  # atomic mass (g/mol)

# Reference geometry (from E_spec_nit.py)
...
# Reference density of Nitrogen at standard conditions (g/cm³)
# This is used for all range calculations since thickness is defined at P0_MBAR
RHO_REF_G_CM3 = 0.00114 # g/cm³ at P0_MBAR and TEMP_K
...
SOURCE_PEAKS_FILE = {
    'pu': 'peaks_by_thickness_nit3.txt',
    'am': 'peaks_by_thickness_nit3.txt',
    'cm': 'peaks_by_thickness_nit3.txt',
    'th': 'peaks_by_thickness_nit4.txt',
}
...
def pressure_to_density_g_cm3(pressure_mbar):
    """
    Convert pressure to Nitrogen gas density using ideal gas law.
    ρ = (P × M) / (R × T)
    ...
    M_N = A_N / 1000.0  # molar mass in kg/mol
    
    # Convert pressure
    P_Pa = pressure_mbar * 100.0  # 1 mbar = 100 Pa
    
    # Ideal gas law: ρ = (P × M) / (R × T)
    rho = (P_Pa * M_N) / (R * TEMP_K)  # kg/m³
...
def analyze_source(source, data_dict, base_dir, output_prefix):
    ...
    print(f"\n{'='*70}")
    print(f"ANALYZING SOURCE (NIT): {source.upper()}")
    print(f"{'='*70}")
    ...
    for i in range(1, len(data_dict['pressure_mbar'])):
        ...
        I_nominal = find_mean_excitation_energy_for_thickness(E1, E2, thickness_cm, Z_N, A_N, RHO_REF_G_CM3)
        
        print(f"  Nominal I = {I_nominal:.2f} eV")
        
        # Propagate uncertainties
        unc_dict = propagate_uncertainty(E1, E2, thickness_cm, E1_unc, E2_unc, pressure_unc, Z_N, A_N, I_nominal)
    ...
    # Print results
    print("=" * 70)
    print(f"RESULTS FOR SOURCE (NIT): {source.upper()}")
    print("=" * 70)
...
def create_source_plots(source, thicknesses, I_values, I_uncs, I_mean, I_total_unc,
    ...
    ax1.set_title(f'Nitrogen gas {source.upper()} - Mean Excitation Energy vs Thickness')
    ax1.legend()
    ...
    ax2.set_title(f'Nitrogen gas {source.upper()} - Distribution of $I$ Values')
    ax2.legend()
    ...
    table.scale(1.0, 1.2)
    ax_tbl.set_title(f'Nitrogen gas {source.upper()} - Mean Excitation Energy and Uncertainties', pad=12)
    plt.tight_layout()
...
def create_combined_plots(all_results_by_source, base_dir, output_prefix, overall_mean, overall_total_unc):
    ...
    colors = {'pu': 'blue', 'am': 'red', 'cm': 'green', 'th': 'purple'}
    ...
    ax.set_title('NITROGEN GAS (NIT) - Combined Mean Excitation Energy vs Thickness (All Sources)', fontsize=13, fontweight='bold')
    ...
    ax.set_title('NITROGEN GAS (NIT) - Combined Distribution of $I$ Values (All Sources)', fontsize=13, fontweight='bold')
    ax.legend(fontsize=10, loc='best')
...
def main():
    """Main analysis routine for Nitrogen gas energy loss study."""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    print("\n" + "="*70)
    print("NITROGEN GAS - MEAN EXCITATION ENERGY ANALYSIS")
    print("="*70)
    print(f"Material: Nitrogen (Z={Z_N}, A={A_N} g/mol)")
    ...
    for source in SOURCES_TO_PROCESS:
        peaks_filename = SOURCE_PEAKS_FILE.get(source, 'peaks_by_thickness_nit3.txt')
        peaks_path = os.path.join(base_dir, peaks_filename)
        data_dict = load_data_for_source(peaks_path, source)
        if data_dict is not None:
            results = analyze_source(source, data_dict, base_dir, 'nit_combined')
    ...
    if len(all_I_values) > 0:
        ...
        print(f"\nFINAL RESULT FOR NITROGEN: I = {overall_mean:.2f} ± {overall_total_unc:.2f} eV")
        print("="*70)
        
        # Create combined plots
        create_combined_plots(all_results_by_source, base_dir, 'nit_combined', overall_mean, overall_total_unc)
...
def save_combined_results(all_results_by_source, base_dir, overall_mean, overall_total_unc):
    ...
    out_path = os.path.join(base_dir, 'ionization_results_nit_combined.txt')
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write('# Mean Excitation Energy Analysis Results for Nitrogen\n')
        f.write(f'# Material: Nitrogen Gas (Z={Z_N}, A={A_N} g/mol)\n')
