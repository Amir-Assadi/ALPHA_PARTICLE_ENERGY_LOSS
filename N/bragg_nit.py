...
# Gas label for plot titles
GAS_LABEL = 'Nitrogen gas'
...
def process_station(base_dir, station_tag):
	...
	for source, res in fit_results.items():
		...
		plt.title(f'{GAS_LABEL} {station_tag.upper()} {source.upper()} - E(x) fitted with spline')
	...
	for source, res in fit_results.items():
		...
		plt.title(f'{GAS_LABEL} {station_tag.upper()} {source.upper()} - Bragg curve from spline fit')
		plt.grid(True, alpha=0.3)
		plt.legend()
		plt.tight_layout()
		plt.show()
...
def main():
	...
	# Process station 3 (nit3)
	process_station(base_dir, 'nit3')
	
	# Process station 4 (nit4) - will skip gracefully if not available
	process_station(base_dir, 'nit4')
