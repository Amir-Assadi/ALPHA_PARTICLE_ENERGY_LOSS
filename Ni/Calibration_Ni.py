...
def main():
    # files to process for nickel experiment
    filenames = [
        'alpha 1xni.txt',
        'alpha 0.5xni.txt',
        'alpha 0.2xni.txt',
    ...
    # Example: PEAK_CHANNELS = {'alpha 1xni.txt': 1600, 'alpha 0.5xni.txt': 800, ...}
    PEAK_CHANNELS = {
        'alpha 1xni.txt': 1660,
        'alpha 0.5xni.txt': 830,
        'alpha 0.2xni.txt': 330,
        'alpha 0.1xni.txt': 165,
    }
    ...
    for idx, fn in enumerate(filenames):
        ...
        scale_label = fn.replace('alpha ', '').replace('ni.txt', '')
    ...
    scale_map = {
        'alpha 1xni.txt': 1.0,
        'alpha 0.5xni.txt': 0.5,
        'alpha 0.2xni.txt': 0.2,
        'alpha 0.1xni.txt': 0.1,
    }
    ...
    for fn in valid_energies_labels:
        ...
        if scale is None:
            # attempt to parse a number like '1xni' from filename
            if '1xni' in fn:
                scale = 1.0
            elif '0.5xni' in fn:
                scale = 0.5
    ...
    # Save calibration parameters to file for use in E_spec_Ni.py
    calib_path = os.path.join(base_dir, 'calibration_params_ni.txt')
    with open(calib_path, 'w') as f:
        f.write('# Calibration parameters for Ni: channel = a * E + c, or E = (channel - c) / a\n')
        f.write(f'a = {a}\n')
