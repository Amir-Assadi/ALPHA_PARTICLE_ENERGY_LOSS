...
# Example: PEAK_CHANNELS = {'T0oneni.txt': 1607, 'T1twoni.txt': 802}
PEAK_CHANNELS = {
	'T0oneni.txt': 1650,
	'T0twoni.txt': 1653,
	'T1oneni.txt': 1542,
...
def extract_thickness(filename):
	"""Extract thickness from filename like 'T0oneni.txt' -> '0 μm' or 'T1oneni.txt' -> '1 μm'"""
	match = re.match(r'^T(?P<th>\d+(?:_\d+)?)(?:one|two)', filename)
	if match:
...
def main():
	...
	calib_params = None
	calib_path = os.path.join(base_dir, 'calibration_params_ni.txt')
	if os.path.isfile(calib_path):
		...
		print(f'Note: calibration_params_ni.txt not found. Energy calibration will not be applied.')
		print(f'      Run Calibration_Ni.py first to generate it.')

	# match filenames like T0oneni.txt, T1oneni.txt, T10twoni.txt etc.
	m = re.compile(r'^T(?P<th>\d+(?:_\d+)?)(?P<trial>one|two)ni\.txt$', re.IGNORECASE)
	...
	out_all_path = os.path.join(base_dir, 'peaks_all_trials_ni.txt')
	...
	out_path = os.path.join(base_dir, 'peaks_by_thickness_ni.txt')
