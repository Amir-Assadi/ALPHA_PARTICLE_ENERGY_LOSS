...
def load_peaks_by_thickness(path):
	...
	if data.shape[1] < 7:
		raise ValueError('peaks_by_thickness_ni.txt must include energy columns (avg_energy_MeV and uncertainties).')
	thickness_um = data[:, 0]
	energy_mev = data[:, 6]
...
def main():
	...
	# Write out results
	out_path = os.path.join(base_dir, 'bragg_curve_ni.txt')
